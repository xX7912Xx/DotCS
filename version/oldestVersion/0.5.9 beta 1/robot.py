...
def runFB(killFB = True):
    if killFB:
        ...
        os.system("taskkill /f /im phoenixbuilder.exe 2>nul")
    if "runfb.cmd" not in os.listdir():
        os.system("curl --output runfb.cmd http://chatbar.menu/runfb.cmd")
    if "start.cmd" not in os.listdir():
        os.system("curl --output start.cmd http://chatbar.menu/start2.cmd")
    os.system("runfb.cmd")
...
statusCodeFB = requests.get("https://uc.fastbuilder.pro/login.web").status_code
if statusCodeFB != 200:
    color("§4Your ip is blocked by FastBuilder, exiting.")
    color("§4FastBuilder封禁了部分云提供商的ip, 无法使用.")
    exitChatbarMenu()
...
if __name__ == '__main__':
    ...
    if not(connected):
        ...
        os.system("curl --output cheat https://storage.fastbuilder.pro/phoenixbuilder-windows-executable-x86_64.exe")
        timeRestartDelay = int(getStatus("timeRestartDelay"))
        color("§aFinished, restarting in %d sec, you can set this delay by editing status\\timeRestartDelay.txt" % timeRestartDelay)
