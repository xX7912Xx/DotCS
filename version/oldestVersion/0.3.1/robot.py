...
if newversion != version:
    ...
    color("§4Finished, restarting.", output = True)
    sys.exit()
else:
...
class NeteaseServerWS(object):
    ...
    def on_message(self, obj, revOri):
        global rev, time_old, playername, msg, needToGet, target, server, connected
        rev_old = rev
        try: 
...
if __name__ == '__main__':
    ...
    while not(connected) and retries <= 15:
        game.start()
        if not(connected):
            color("§4Retrying: "+str(retries)+", Is it that FB program is still starting, not connected to the Netease Minecraft Server?", output = True)
            retries += 1
    if not(connected):
        os.system("start /wait update.cmd")
    print("Exiting.")
    game.ws.close()
    os.system("taskkill /f /im phoenixbuilder.exe")
    sys.exit()
