...
def color(text, output = False, end = "\n"):
    ...
    if output:
        print(text, end = end)
    return text
color('§b".命令"系统 - 租赁服聊天栏菜单\n感谢CLP大佬提供的修改版FastBuilder程序.\n辅助处理脚本编写者: art, 7912', output = True)
color('§b用户交流群: 467443403', output = True)
import time
time.sleep(2)
print("Importing.")
import socket
...
import urllib
import urllib.parse
color("§eChecking Updates. Your version: %s" % version, output = True)
...
if newversion != version:
    color("§eLatest version: %s, downloading." % newversion, output = True)
    time.sleep(1)
    os.system("curl --output robot_new.exe http://chatbarmenu.com/robot.exe")
    color("§aFinished, restarting.", output = True)
    sys.exit()
else:
    color("§aYou are running the latest version.", output = True)
if allow == "false":
    reason = status[2].split("msg: ")[1]
    color("§4%s" % reason, output = True)
    os.system("taskkill /f /im phoenixbuilder.exe")
    sys.exit()
    exit()
color("§eLoading plugins.", output = True)
time.sleep(0.2)
...
for filename in pluginlist:
    try:
        if filename[-10:-3]=="cmdsrun":
            color("§eNow loading: %s" % filename, output = True)
            ...
            try:
                ...
                color("§4Ignored: %s, reason: %s" % (filename, err), output = True)
                pluginIgnoredNum += 1
            ...
            color("§4Ignored: %s, reason: Wrong filename type." % filename, output = True)
            pluginIgnoredNum += 1
        ...
        time.sleep(0.05)
color("§aLoaded all plugins.", output = True)
time.sleep(0.2)
print("Loading defs.")
...
class NeteaseServerWS(object):
    ...
    def on_open(self, obj):
        global connected
        color("§aConnected to FB program.", output = True)
        color("§eApplying onopen plugins.", output = True)
        if not(connected):
        ...
        if pluginIgnoredNum >= 1:
            ...
            for i in pluginIgnoredNameList:
                ...
                FinishedLoadingNum += 1
                time.sleep(0.02)
        color("§aChatbar Menu program started successfully.", output = True)
...
if __name__ == '__main__':
    ...
    color("§eConnecting to FB program, please wait.", output = True)
    connected = False
    retries = 1
    while not(connected) and retries <= 15:
        game.start()
        if not(connected):
            color("\r§4Retrying: "+str(retries)+", Is it that FB program is still starting, not connected to the Netease Minecraft Server?", output = True, end = "")
            retries += 1
    if not(connected):
        color("\n§4Cannot connect to FB program.", output = True)
        color("§eMaybe FB is outdated, updating FB.", output = True)
