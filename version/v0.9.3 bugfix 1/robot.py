...
try:
    ...
    def getPos(targetName: str) -> dict:
        ...
        result = sendcmd("/querytarget %s" % targetName, True)["OutputMessages"][0]
    ...
    def processMsg() -> None:
        ...
        """
        global msgLastRecvTime, XUID2playerName, entityRuntimeID2playerName, timeSpentFBRun, msgRecved, msgList, server, server, connID, timeGameH, timeGameM, timeGameHstr, timeGameMstr, timesErr, robotname, allplayers, needToGetMainhandItem, itemMainhand, needToGetArmorItem, itemArmor, needToGetMainhandAndArmorItem, itemMainhandAndArmor, targetArmor, targetMainhand
        time_old = time.time()
        while not msgRecved:
                ...
                if packetType == 9:
                    ...
                    if "executing" in msg:
                        ...
                        if robotname == "":
                            ...
                            sendcmd("/execute @a ~ ~ ~ /tell %s getXUID" % robotname)
                            try:
                    ...
                    # 处理文字信息.
                        ...
                        # 处理玩家在聊天栏发送的信息, tell信息以及say信息
                            ...
                            if textType == 7 and msg == "getXUID":
                                if playername in allplayers:
                                    XUID2playerName[jsonPkt["XUID"]] = playername
                            else:
