...
def blackListCheck():
    try:
        ...
        result = requests.get("http://chatbar.menu:7913/api?type=serverNumberBanSearch&serverNumber=%s" % server, timeout = 5).json()
...
def setStatus(statusname, status):
    try:
        ...
        try:
            ...
            return status
def load_function_first():#检测function文件夹是否存在
    "检测function文件夹是否存在,不存在则创建,该方法没有返回值"
    if os.path.isdir("function") == False:
        os.mkdir("function") 
def function_first_run(function:str,playername:str)->None:#检测function文件是否存在并且执行
    "检测function文件是否存在并且执行"
    load_function_first()
    if playername  in player_list_smoll():
        function_list = function.strip().split(" ")
        function_list_comment = 0
        for i in function_list:     
            if i[-11:] !=".mcfunction":
                i =i+".mcfunction"
            if os.path.isfile("function\\"+i) == False:
                sendcmd("/tellraw "+playername+""" {"rawtext":[{"text":"<§4Error§r> """+"§4函数文件:"+i+"""不存在!!!"}]}""")
            elif i.endswith(".mcfunction") ==False:
                sendcmd("/tellraw "+playername+""" {"rawtext":[{"text":"<§4Error> """+""+i+"""不是有效的.mcfunction文件"}]}""")
            else:
                a = open("function\\"+i,"r",encoding="UTF-8")
                for line in a.readlines():
                    if line.replace(re.sub(r'#.*$', "", line),"")!="":
                        continue
                    elif re.sub(r'#.*$', "", line)!="" and "#" in line:
                        line = line.replace(re.sub(r'#.*$', "", line),"")
                    if line[-1] =="\n":line=line[0:-1]
                    line_gammer = line.split(" ")
                    if len(line_gammer) != 1 and line_gammer[0] == "function":
                        function_first_run(line_gammer[1],playername)
                    elif line_gammer[0] == "function":
                        sendcmd("/tellraw "+playername+''' {"rawtext":[{"text":"§4语法错误:function [文件] 缺少文件参数"}]}''')
                    if len(line_gammer) != 1 and line_gammer[0] == "time_late":
                        time.sleep(float(line_gammer[1]))
                    elif line_gammer[0] == "time_late":
                        sendcmd("/tellraw "+playername+''' {"rawtext":[{"text":"§4语法错误:time_late 缺少文件参数"}]}''')
                    if line[0] !="/":
                        line = "/" + line
                    sendcmd("/execute "+playername +"~~~ "+line)
                    function_list_comment +=1
        if function_list_comment !=0:
            sendcmd("/tellraw "+playername+''' {"rawtext":[{"text":"成功执行了'''+str(function_list_comment)+'''个功能项"}]}''')
        else:
            sendcmd("/tellraw "+playername+''' {"rawtext":[{"text":"§4未成功执行任何功能项"}]}''')
    else:
        function_list = function.strip().split(" ")
        function_list_comment = 0
        for i in function_list:     
            ...
            if os.path.isfile("function\\"+i) == False:
                ...
                for line in a.readlines():
                    ...
                    if len(line_gammer) != 1 and line_gammer[0] == "function":
                        ...
                        sendcmd("/tellraw @a "+''' {"rawtext":[{"text":"§4语法错误:function [文件] 缺少文件参数"}]}''')
                    if line[0] !="/":
                        line = "/" + line
                    sendcmd(line)
                    function_list_comment +=1
