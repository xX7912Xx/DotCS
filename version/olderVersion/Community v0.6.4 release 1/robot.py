...
r"""
# 设置版本号
version = "Community v0.6.4 release 1"
    ...
    try:
        with open("nohup.out", "r", encoding = "utf-8") as file:
            ...
            if strInList("ERROR", FBlog):
                ...
                FBerror = True
                if strInList("Unauthorized rental server number", FBlog):
...
if __name__ == '__main__':
    ...
    while not(connected) and retries <= 20:
        try:
            FBlog = FBlogRead()
            while not(strInList(("Server: %s" % server), FBlog)):
                time.sleep(0.1)
                FBlog = FBlogRead()
            color("§bFB output:§6 Server: %s" % server)
            while not(strInList("Auth pass", FBlog)):
                FBlog = FBlogRead()
                time.sleep(0.1)
            color("§bFB output:§r Auth pass")
            while not(strInList("Successfully created minecraft dialer.", FBlog)):
                FBlog = FBlogRead()
                time.sleep(0.1)
            color("§bFB output:§6 Successfully created minecraft dialer.")
            while not(strInList("Transfer: server start successfully", FBlog)):
                FBlog = FBlogRead()
                time.sleep(0.1)
            color("§bFB output:§6 Transfer: server start successfully")
            game.start()
        except Exception as err:
            if connected:
                print(str(err))
