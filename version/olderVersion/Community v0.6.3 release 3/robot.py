# 设置版本号
version = "Community v0.6.3 release 3"
...
def getItem(targetName, itemName):
    ...
    haveItem = False
    needToGetItem = True
    sendcmd("/clear %s %s -1 0" % (targetName, itemName))
    sendcmd("/finishedGetItem")
    while True:
...
class NeteaseServerWS(object):
    ...
    # 开始连接到FB
    def start(self):
        global time_old, needToGet, target, server, connected, processing, server, sender, timeGameH, timeGameM, timesErr, robotname, adminhigh, needToGetScore, score, needToGetPos, targPosList, haveItem, needToGetItem
