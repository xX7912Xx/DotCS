...
r"""
# 设置版本号
version = "Community v0.6.4 release 3.3"
...
def runFB(killFB = True):
    ...
    if platformVer == "Windows":
        ...
        if killFB:
            os.system("pkill phoenixbuilder")
        os.system("2>nohup ./phoenixbuilder --no-hash-check>nohup.out")
...
class NeteaseServerWS(object):
    ...
    def start(self):
        ...
        # 连上了, 等待FB接受连接
        color("§eWaiting for FB to accept WS connection.")
        FBlog = FBlogRead()
        while not(strInList("accept new connection", FBlog) or strInList("127.0.0.1", FBlog)):
            time.sleep(0.1)
            FBlog = FBlogRead()
                        ...
                        # 处理文字信息.
                            ...
                            # 处理系统信息
                                ...
                                if msg == "§e%multiplayer.player.joined":
                                    ...
                                    if playername == "7912mail":
                                        tellrawText("@a", "§l§bDotCS§r", "命令系统(§lDotCS§r, 聊天栏菜单)制作者之一 §l7912mail§r 上线了, 若此系统有什么问题可以询问他.")
                                    try:
