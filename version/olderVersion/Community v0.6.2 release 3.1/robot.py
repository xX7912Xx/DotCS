version = "Community v0.6.2 release 3.1"
...
class NeteaseServerWS(object):
    ...
    def start(self):
        global time_old, needToGet, target, server, connected, processing, server, sender, timeGameH, timeGameM, timesErr, robotname, adminhigh
                    ...
                    if revType == "Text":
                        ...
                        if "executing" in msg:
                            ...
                            if textType == 9: #过滤tellraw信息
                                color("["+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"] "+str(textType)+" "+msg.replace('{"rawtext":[{"text":"', "").replace('"}]}', "").replace('"},{"text":"', "").replace(r"\n", "\n"+"§r["+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"] "+str(textType)+" ").replace("§l", ""), end = "")
