...
try:
            ...
            with open(path, 'wb') as file:
                try:
                    for data in response.iter_content(chunk_size = freshSize):
                        ...
                        color("§e正在下载: %.2f%%, %s / %s, %s/s" % (fileDownloadedSize/fileSize*100, Byte2KB(fileDownloadedSize), Byte2KB(fileSize), Byte2KB(speedDownloadCurrent)), replace = True, info = "§e 下载 ")
                except Exception as err:
    ...
    def log(text: str, filename: str = None, mode: str = "a", encoding: str = "utf-8", errors: str = "ignore", output: bool = True, sendtogamewithRitBlk: bool = False, sendtogamewithERROR: bool = False, sendtogrp: bool = False, info = None) -> None:
        """
        记录日志的函数
    ...
    def getGameTimeRepeat(self) -> None:
        ...
        while True:
            try:
                ...
                timeStart = time.time()
                timeGet = time.time()
                gameTimeTick = (int(sendcmd("/time query daytime", True, timeout = 20, error = "ignore")["OutputMessages"][0]["Parameters"][0])+6000)%24000
    ...
    def othersRepeat(self) -> None:
        ...
        while True:
            ...
            try:
                if connected:
                    updateCheck()
                    allplayers = getTarget("@a[rm=0.001]", timeout = 60, error = "ignore")
            except Exception as err:
                pass
            finally:
    ...
    def revPacket(self) -> None:
        ...
        while True:
            try:
                ...
                color("§e正在连接到 FastBuilder, %.2fs" % spendTime, replace = True, replaceByNext = True, info = "§c 加载 ")
        # 连上了
