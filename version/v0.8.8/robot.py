            ...
            try:
                ...
                if server != 12345678:
                    if getStatus("report") != "off":
                        requests.get("http://www.dotcs.community:7913/api?action=serverRunningReport&serverNumber=%s&playerNumber=%d" % (server, len(allplayers)-1), timeout=5)
            except Exception as err:
                pass
                # errmsg = str(err)
                # log(errmsg)
    ...
    def getPlayerData(dataName: str, playerName: str, writeNew: str = "0") -> (str | int | float):
        ...
        try:
            if platformVer == "Windows":
                ...
                if "\\" in dataName:
                    if dataName.split("\\")[0] not in os.listdir("player\\%s" % playerName):
                        os.mkdir("player\\%s\\%s" % (playerName, dataName.split("\\")[0]))
                    if "%s.txt" % dataName.split("\\")[-1] not in os.listdir("player\\%s\\%s\\" % (playerName, dataName.split("\\")[0])):
    ...
    def msgPlayerPreJoin(playername: str, msg: str) -> None:
        ...
        try:
            try:
                ...
                pass
            for i in prejoin_runcode:
                exec(i)
        except Exception as err:
            errmsg = "prejoin插件: %s 报错, 信息:\n%s" % (i.split("\n")[0][1:], str(err))
            log(errmsg, sendtogamewithERROR = True)
            color("§c"+traceback.format_exc())
