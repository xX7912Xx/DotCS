...
try:
    ...
    def countdown(delay, msg = None):
        ...
        while delay >= 0.01:
            ...
            time.sleep(0.01)
        # color(" ", replace = True, show = True)

    
    # 便捷退出命令系统的函数
    def exitChatbarMenu(killFB = True, delay = 3, reason = None):
        global exiting
        if not exiting:
            if reason is None:
                color("§eExiting.")
    ...
    def updateCheck():
        ...
        if newversion != version:
            ...
            time.sleep(1)
            if platformVer == "Windows":
                thread.start_new_thread(updateSizeTell, ())
    ...
    def updateSizeTell():
        ...
        while not ("robot_new.exe" in os.listdir()):
            time.sleep(0.1)
        color("§eReceiving file.")
        while True:
            try:
                color("§eDownloaded: %dKB." % (os.path.getsize("robot_new.exe")/1024), replace = True, replaceByNext = True)
            except:
                pass
            finally:
                time.sleep(0.1)
    ...
    # 获取玩家本地数据
    def getPlayerData(dataName, playerName, writeNew = "0"):
        try:
            if platformVer == "Windows":
    ...
    def setPlayerData(dataName, playerName, dataValue, writeNew = "0"):
        try:
            if platformVer == "Windows":
                if playerName not in os.listdir("player"):
                    os.mkdir("player\\%s" % playerName)
    ...
    def addPlayerData(dataName, playerName, dataValue, dataType = "int", writeNew = "0"):
        try:
            if dataType == "int":
    ...
    def msgPlayerSend(playername, msg):
        try:
    ...
    # 命令系统启动后玩家死亡时调用1次的函数
    def msgPlayerDeath(playername, msg, killer):
        try:
    ...
    # 命令系统启动后玩家加入时调用1次的函数
    def msgPlayerJoin(playername, msg):
        try:
            try:
                int(playername[0])
                sendcmd('/kick "%s" §c抱歉, 因命令系统(DotCS Community)作者7912技术不足, 暂不允许数字开头的玩家进服.' % "emm")
            except:
                pass
    ...
    # 命令系统启动后玩家退出时调用1次的函数
    def msgPlayerLeave(playername, msg):
        try:
                ...
                with open("nohup.out", "r", encoding = "utf-8") as file:
                    ...
                    if FBlogNew != []:
                        ...
                        if strInList("ERROR", FBlogNew):
                            if strInList("Unauthorized rental server number", FBlogNew) or strInList("对应租赁服号尚未授权", FBlogNew) or strInList("Server not found", FBlogNew) or strInList("租赁服未找到", FBlogNew):
                                ...
                                color("§a成功设置服号")
                            elif strInList("Unauthorized rental server number", FBlogNew) or strInList("对应租赁服号尚未授权", FBlogNew):
                                pass
                            if connected:
                                FBkill()
                                # restartFB()
    ...
    msgRecved = False
    def processMsg():
        global timeSpentFBRun, msgRecved, msgList, needToGet, target, server, connected, server, connID, timeGameH, timeGameM, timeGameHstr, timeGameMstr, timesErr, robotname, needToGetScore, score, needToGetPos, targPosList, haveItem, needToGetItem, allplayers, needToGetMainhandItem, itemMainhand, targetMainhand
        time_old = time.time()
        while not msgRecved:
            ...
            try:
                if msgList == []:
                    ...
                    try:
                        ...
                        continue
                    # if packetType == 12 or packetType == 31:
                    #     print(jsonPkt)
                    ...
                    # elif packetType == 79:
                        # print(jsonPkt)
                    # if "death" in str(jsonPkt) or "失败了" in str(jsonPkt) or "7912mail" in str(jsonPkt):
                    #     print(jsonPkt)
                    # {"TextType":1,"NeedsTranslation":false,"SourceName":"7912mail","Message":"发信息测试","Parameters":null,"XUID":"","PlatformChatID":"","PlayerRuntimeID":""}
                    # {"CommandOrigin":{"Origin":5,"UUID":"e78ce87f-b4e1-11ec-adeb-00ff76d0ff27","RequestID":"96045347-a6a3-4114-94c0-1bc4cc561694","PlayerUniqueID":0},"OutputType":4,"SuccessCount":0,"OutputMessages":[{"Success":false,"Message":"commands.scoreboard.objectiveNotFound","Parameters":["time"]}],"DataSet":"{\n   \"statusCode\" : -2147352576\n}\n"}
                    # 已经实现类型数据的解析
                    # 处理文字信息.
                    if packetType == 9:
                        # 初始化
    ...
    def revMsg():
        global timeSpentFBRun, msgRecved, msgList, timesErr, connected, connID, reconnecting, timeStartFBRun, FBlog
    ...
    if __name__ == "__main__":
        version = "Community v0.7.0 release 1" # 设置版本号
            ...
            try:
                if filename[-10:-3]=="cmdsrun":
                    ...
                    if filename[0:4] == "enc_":
                        ...
                        cmds_runcode.append("#%s\n%s" % (filename, plugincode.read().replace('% getCoin(playername)', '% getPlayerData("coin", playername)')))
                    plugincode.close()
                    try:
