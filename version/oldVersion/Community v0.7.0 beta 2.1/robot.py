...
try:
    ...
    def repeating4():
        ...
        while True:
            try:
                timesErr = 0
                allplayers_get = getTarget("@a")
                if "获取超时" not in allplayers_get:
                    allplayers = allplayers_get
                updateCheck()
                if server != 12345678:
    ...
    # 命令系统启动后玩家死亡时调用1次的函数
    def msgPlayerDeath(playername, msg):
        try:
    ...
    msgRecved = False
    def processMsg():
        global timeSpentFBRun, msgRecved, msgList, needToGet, target, server, connected, server, connID, timeGameH, timeGameM, timeGameHstr, timeGameMstr, timesErr, robotname, needToGetScore, score, needToGetPos, targPosList, haveItem, needToGetItem, allplayers
        time_old = time.time()
        while not msgRecved:
                        ...
                        # 处理文字信息.
                            ...
                            # 处理系统信息
                                ...
                                # 处理玩家死亡信息
                                    ...
                                    thread.start_new_thread(msgPlayerDeath, (playername, msg))
    ...
    if __name__ == "__main__":
        version = "Community v0.7.0 beta 2.1" # 设置版本号
