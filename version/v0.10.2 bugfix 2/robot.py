...
try:
        ...
        for i in threadList[:]:
            try:
                if i.name == "与NekoMaid网站通信":
                    NekoMaidMsg(["NekoMaid:console:log", {"level": "INFO", "logger": "net.minecraft.server.v1_16_R3.DedicatedServer", "msg": "%s§r" % "["+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"] "+text.replace("'", 'unchanged'), "time": time.time()*1000}], "42", i.data["conn"], True)
            except:
                pass
    ...
    def processPacket(self) -> None:
        ...
        """
        global msgLastRecvTime, XUID2playerName, timeSpentFBRun, msgRecved, msgList, server, server, connID, robotname, allplayers, connected
        try:
            while not msgRecved:
                time.sleep(0.01)
            connected = True
            ...
            result = sendcmd("/time add 0", True, 10)["OutputMessages"][0]
            if result["Success"] == False:
            ...
            robotname = getTarget("@s", timeout = 10)[0]
            if ")" in robotname:
            ...
            allplayers = getTarget("@a", timeout = 10)

            color("§eGetting players' XUID.")
            for i in allplayers:
                result = sendcmd('/querytarget @a[name="%s"]' % i, True, 10)["OutputMessages"][0]
                if result["Success"] == False:
    ...
    def revPacket(self) -> None:
        ...
        你不应该使用此函数, 命令系统会在运行FastBuilder后以新线程启动此函数
        """
        global msgLastRecvTime, timeSpentFBRun, msgRecved, msgList, connID, timeStartFBRun, FBlog
    ...
    if __name__ == "__main__":
        version = "v0.10.2 bugfix 2" # 设置版本号
        ...
        with open("server.txt") as file:
            ...
            if server == "12345678":
                ...
                if FBconfig["serverNumber"] == "":
                    exitChatbarMenu(reason = "Netease server number error")
                server = FBconfig["serverNumber"]
            ...
            if serverPassword == "123456":
                ...
                if len(FBconfig["serverPassword"]) != 6 or FBconfig["serverPassword"] == "123456":
                    exitChatbarMenu(reason = "Netease server password error")
                serverPassword = FBconfig["serverPassword"]
        ...
        createThread("与命令系统WebSocket服务器通信", data = {"address": "ws://www.dotcs.community:7912/WebSocket?"}, func = WebSocketClient)
        ...
        if not os.path.isfile("account.txt"):
            ...
            if choice == "l":
                ...
                exitChatbarMenu(reason = "unknown choice")
            ...
            try:
                ...
                exitChatbarMenu(reason = "load account file error")
            result = WebSocketDotCS.send({"action": "login", "account": account, "password": password}, showText = "§eLoginning.")
            if result["status"] == "fatal":
                color("§4WebSocket server process error, reason: %s" % result["reason"])
                exitChatbarMenu(delay = 60)
        ...
        for i in pluginList:
            ...
            try:
                if i.enable and pluginRunType in i.pluginCode:
                    if faststart:
                        if i.pluginName == "商店.py":
                            exec(i.pluginCode[pluginRunType].replace("time.sleep(", "("))
                        elif i.pluginName == "MusicPlayer.py":
                            exec(i.pluginCode[pluginRunType].replace('countdown(3, "§e请阅读说明")', ""))
                        else:
                            exec(i.pluginCode[pluginRunType])
                    else:
                        exec(i.pluginCode[pluginRunType])
    ...
    # except:
    #     import traceback
    #     print("\033[0;37;31m%s\033[0m" % traceback.format_exc())
