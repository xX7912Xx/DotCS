						...
						if err := decoder.Decode(&msg); err != nil {
							...
							if enableServerChatroom {
								...
								case "msg":
									o.Frame.GetGameControl().SayTo("@a", fmt.Sprintf("%v %v", msg.ServerName, msg.Data))
								case "connected":
									o.Frame.GetGameControl().SayTo("@a", fmt.Sprintf("§b[§e通讯终端§b]§r §r<§e系统§r>§r %v重新搜索到了!", msg.ServerName))
								case "disconnected":
									o.Frame.GetGameControl().SayTo("@a", fmt.Sprintf("§b[§e通讯终端§b]§r §r<§e系统§r>§r %v信号丢失", msg.ServerName))
								case "player.join":
									o.Frame.GetGameControl().SayTo("@a", fmt.Sprintf("§b[§e通讯终端§b]§r §r<§e系统§r>§r %v §g%v 加入了游戏", msg.ServerName, msg.Data))
								case "player.left":
									o.Frame.GetGameControl().SayTo("@a", fmt.Sprintf("§b[§e通讯终端§b]§r §r<§e系统§r>§r %v §g%v 退出了游戏", msg.ServerName, msg.Data))
								case "get_data_serverlist":
									o.Frame.GetGameControl().SayTo("@a", fmt.Sprintf("§b[§e通讯终端§b]§r §r<§e系统§r>§r §a目前与您互通的服务器§7:§e %v", msg.Data))
								}
							}
							switch msg.Action {
							case "consolemsg":
