...
class __init__(__main__.plugin):
    ...
    async def initialize(self):
        ...
        if "plugin.DotCS_pip" not in __main__.PLUGIN_CLASS:
            # 检测是否有此插件
            self.color("§4玩家封禁插件依赖 §eDotCS_pip §4插件(如果您已经安装了,请勿改名)!请前往 §bhttps://github.com/xX7912Xx/DotCS/tree/main/plugin §4进行下载")
            del __main__.PLUGIN_CLASS["plugin.玩家封禁"]
        elif "plugin.sqlite3_api" not in __main__.PLUGIN_CLASS:
            # 检测是否有此插件
            self.color("§4玩家封禁插件依赖 §esqlite3_api §4插件(如果您已经安装了,请勿改名)!请前往 §bhttps://github.com/xX7912Xx/DotCS/tree/main/plugin §4进行下载")
            del __main__.PLUGIN_CLASS["plugin.玩家封禁"]
        else:
            if "plugin.Mysql_api" not in __main__.PLUGIN_CLASS:
                # 检测是否有此插件
                self.color("§e玩家封禁插件支持使用 §eMysql_api §4插件(如果您已经安装了,请勿改名)!")
    ...
    def color(self,text):
        self.colorf("§e组件§7-§b玩家封禁 §6> "+text)
