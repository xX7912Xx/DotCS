...
class __init__(__main__.plugin):
    ...
    async def initialize(self):
        ...
        if "plugin.DotCS_pip" not in __main__.PLUGIN_CLASS:
            # 检测是否有此插件
            self.color("§4平面道路导航_图片识别_前置api插件依赖 §eDotCS_pip §4插件(如果您已经安装了,请勿改名)!请前往 §bhttps://github.com/xX7912Xx/DotCS/tree/main/plugin §4进行下载")
            del __main__.PLUGIN_CLASS["plugin.平面道路导航_图片识别_前置api"]
        else:
            try:
                import cv2
            except:
    ...
    def color(self,text):
        self.colorf("§e组件§7-§b平面道路导航_图片识别_前置api §6> "+text)
