...
class __init__(__main__.plugin):
    ...
    async def initialize(self):
        ...
        if "plugin.DotCS_pip" not in __main__.PLUGIN_CLASS:
            # 检测是否有此插件
            self.color("§4群服互通插件依赖 §eDotCS_pip §4插件(如果您已经安装了,请勿改名)!请前往 §bhttps://github.com/xX7912Xx/DotCS/tree/main/plugin §4进行下载")
            del __main__.PLUGIN_CLASS["plugin.群服互通"]
        else:
            try:
                import requests
            except:
                self.color("§e正在安装依赖库§7:§b requests")
                __main__.PLUGIN_CLASS["plugin.DotCS_pip"].install("requests")
                self.color("§a安装依赖库完成§7:§b requests")
                import requests
    ...
    def color(self,text):
        self.colorf("§e组件§7-§b群服互通 §6> "+text)
