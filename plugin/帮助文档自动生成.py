...
class __init__(__main__.plugin):
    ...
    async def initialize(self):
        ...
        if "plugin.DotCS_pip" not in __main__.PLUGIN_CLASS:
            # 检测是否有此插件
            self.color("§4帮助文档自动生成插件依赖 §eDotCS_pip §4插件(如果您已经安装了,请勿改名)!请前往 §bhttps://github.com/xX7912Xx/DotCS/tree/main/plugin §4进行下载")
            del __main__.PLUGIN_CLASS["plugin.帮助文档自动生成"]
        else:
            try:
                import qrcode
            except:
    ...
    def color(self,text):
        self.colorf("§e组件§7-§b帮助文档自动生成 §6> "+text)
