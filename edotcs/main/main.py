...
class EDotCS:
    ...
    async def Menu(self,player:str,menu:list[str]):
        "接受MC菜单消息"
        pass
    async def Player_Whisper(self,player:str, message:str):
        "接收MC私聊消息"
        pass
        #self.logger.info("退出服务器数据包",str(player))
    async def BlockActorData(self,Position:type.BlockPos.BlockPos, data:Any):
        "NBT方块Data数据包"
        pass
    ...
    async def listen(self):
        ...
        async with self.session.ws_connect("/dotcs/v8/") as ws:
            ...
            #     optional:str recommendation=4;//插件介绍
            #     optional:str menu_word=5;// 插件菜单关键词(以htp为例, .htp )
            #     optional:str menu_recommendation=6;// 插件菜单关键词介绍
            # }           
            ...
            async for msg in ws:
                ...
                if msg.type == aiohttp.WSMsgType.BINARY:
                    match msg.data[0]:
                        ...
                        case 5:
                            message = grpc.edotcs_pb2.Player_Whisper()
                            message.ParseFromString(msg.data[1:],)
                            self.logger.info("[私聊] <%s> %s"%(message.Player,message.Message))
                            await self.Player_Whisper(message.Player,message.Message)
                        case 6:
                            message = grpc.edotcs_pb2.BlockActorData()
                            message.ParseFromString(msg.data[1:],)
                            Position = type.BlockPos.BlockPos(
                                x=message.Position.X,
                                y=message.Position.Y,
                                z=message.Position.Z,
                            )
                            try:
                                await self.BlockActorData(Position,orjson.loads(message.NBT_Data))
                            except Exception as e:
                                self.logger.error("BlockActorData 错误:",str(e))
                        case 7:
                            message = grpc.edotcs_pb2.System_Message()
                            message.ParseFromString(msg.data[1:],)
                            await self.System_Message(message.NeedsTranslation,message.SourceName,message.Message,message.Parameters,message.XUID,message.PlatformChatID,message.PlayerRuntimeID)
                        case _:
