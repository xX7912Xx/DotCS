...
在我们开始学习插件编写和函数使用前, 我需要告诉你这个妙招:

成功启动命令系统后, 我们可以在其窗口便捷执行指令到租赁服或者执行 Python 代码.

这也许对我们将要学习的插件编写, 函数使用, 甚至排查错误等有较大帮助.

> 若输入的第一个字符为 **"/"**, 则执行指令.
>
> 其他情况下, 执行 Python 代码.

![console](image/DotCS插件/console.png)

也许差不多了..?, 让我们正式开始了解命令系统的插件吧.

---

### 0. 插件格式

哦, 对了, 记住插件的基本格式也很重要. 如果格式错误, 命令系统就无法在对应的时刻执行插件代码.
...
和大多数语言一样, 我们首先学会用插件在社区版控制台输出内容, 这里用到社区版的输出函数: color.
...
> 在社区版中, 使用print进行输出还是会使用color函数
>
> 建议使用color函数进行输出. Python 原版的 print 输出函数被更改为 print_Py (不建议使用)
...
感觉.. 说明有这么长..? 没关系, 我们现在只需使用最简单的cmd参数.
